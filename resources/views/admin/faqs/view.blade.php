    ...
    <section class="panel">
          <div class="row">
             <div class="col-md-12 col-xs-6">
              <header class="panel-heading">Faqs
                    <!-- <a href="{{url('/admin/download/supercategory')}}" class="pull-right btn btn-sm btn-info" style="margin: 2px;">
                      <i class="fa fa-download" aria-hidden="true"></i>Download
                    </a> -->
                    <a href="{{url('/admin/faqs/create')}}" class="pull-right btn btn-sm btn-info" style="margin: 2px;">
                      ...
                      <tbody>
                        ...
                        @foreach ($record as $index=>$key) 
                            <tr class='item<?= $key->id;?>'>
                              ...
                              <td>{{$key->question}}</td>
                              <td>{{$key->answer}}</td>
                              <td>{{$key->question_hindi}}</td>
                              <td>{{$key->answer_hindi}}</td>
                              <td>
                                <label class="switch">
                                  <input type="checkbox" id="status{{$key->id}}" class="updatestatus" data-mode="FaqsSchema" {{ ($key->status == 'Yes' ) ? 'checked' : '' }} >
                              ...
                              <td class="visible-md visible-lg">
                                  <div class="">
                                  <a  class="btn btn-sm btn-info show-tooltip act-no" title="Edit" href="{{ URL::to('admin/faqs/'.$key->id.'/edit') }}"  ><i class="fa fa-edit"></i></a>
