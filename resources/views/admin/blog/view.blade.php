    ...
    <section class="panel">
          <div class="row">
             <div class="col-md-12 col-xs-6">
              <header class="panel-heading">Blog
                    <!-- <a href="{{url('/admin/download/category')}}" class="pull-right btn btn-sm btn-info" style="margin: 2px;">
                      <i class="fa fa-download" aria-hidden="true"></i>Download
                    </a> -->
                    <a href="{{url('/admin/blog/create')}}" class="pull-right btn btn-sm btn-info" style="margin: 2px;">
                      ...
                      <tbody>
                        ...
                        @foreach ($record as $index=>$key) 
                            <tr class='item<?= $key->id;?>'>
                              ...
                              <td>{{$key->keyword}}</td>
                              <td>{!! Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $key->created_at)->format('j M, Y g:i A');!!}</td>
                              <td>
                                <label class="switch">
                                  <input type="checkbox" id="status{{$key->id}}" class="updatestatus" data-mode="Blog" {{ ($key->status == 'Yes' ) ? 'checked' : '' }} >
                                  <span class="slider round"></span>
                                </label>
                              </td>
                              <td class="visible-md visible-lg">
                                  <div class="">
                                  <a  class="btn btn-sm btn-info show-tooltip act-no" title="Edit" href="{{ URL::to('admin/blog/'.$key->id.'/edit') }}"  ><i class="fa fa-edit"></i></a>
